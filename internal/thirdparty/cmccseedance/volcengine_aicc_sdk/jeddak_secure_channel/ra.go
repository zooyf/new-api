...
func attestLocal(token string, config ClientConfig) (string, error) {
	// Use config.RaAuthToken if token is not provided
	authToken := token
	if authToken == "" {
		authToken = config.RaAuthToken
	}

	// Generate timestamp
	timestamp := fmt.Sprintf("%d", time.Now().Unix())

	// Generate nonce (16 characters to match Python version)
	nonce, err := generateNonce(8)
	if err != nil {
		return "", fmt.Errorf("generate nonce: %w", err)
	}

	// Build request body with only nonce
	...
	log.Printf("request ra, url=%s, header=%v, params=%s", config.RaURL, req.Header, string(jsonBody))

	// Send request
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		log.Printf("RA Network error: url=%s, err=%v", config.RaURL, err)
	...
	// Try to get Result or ResponseResult (like Python version)
	var raResults map[string]interface{}
	if result, ok := data["Result"].(map[string]interface{}); ok {
		raResults = result
	} else if result, ok := data["ResponseResult"].(map[string]interface{}); ok {
		raResults = result
	} else {
...
func extractPublicKey(raResults map[string]any) (string, error) {
	for _, entry := range raResults {
		raInfo, ok := entry.(map[string]any)
		if !ok {
			continue
		}
		...
		keyInfo, ok := raInfo["key_info"].(map[string]any)
		if !ok {
...
func validateRaInfo(raInfo map[string]any) bool {
	// Check for evidence
	if evidence, ok := raInfo["evidence"].(string); ok {
		var evidenceJSON map[string]any
